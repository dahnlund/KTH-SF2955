...
driving = unidrnd(5,1,N); %random driving commands chosen uniformly 
driving_vector = zeros(1,n); 
driving_vector(1) = mode(driving); %initialize the drivning vecotr
...
for t = 2:n
    ...
    if resampling == true
        ind = randsample(N,N, true, w);
        driving = drive(cp,driving,N);
        driving_vector(t) = mode(driving);
        part = phi * part(:,ind) + ksi_z * Z + ksi_w * randn(2,N)*sigma;
        l = log(p(part, Y(:,t)));
    else
        part = phi * part+ ksi_z * Z + ksi_w * randn(2,N)*sigma;
...
%% Most probbable driving command at all time steps 
figure
histogram(driving_vector)
title('Driving command at all time steps')
xticklabels({'[0; 0]', '[3.5; 0]', '[0; 3.5]', '[0; -3.5]', '[-3.5; 0]'})

plot(1:n, driving_vector, '.')
title('Driving command at all time steps')
xlabel('Time steps')
ylabel('Driving command')
...
%%
function [driving] = drive(cp, driving,N)
    prob = rand(1,N);
    row = cp(driving,:)';
    
    driving = 1 * (prob < row(1,:)) + 2 * (prob < row(2,:) & prob > row(1,:))... 
                    + 3 * (prob < row(3,:) & prob > row(2,:)) ...
                    + 4 * (prob < row(4,:) & prob > row(3,:)) ...
                    + 5 * (prob < row(5,:) & prob > row(4,:));
end